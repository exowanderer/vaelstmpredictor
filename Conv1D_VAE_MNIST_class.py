...
def build_hidden_conv_layers(filter_sizes, kernel_sizes, input_layer, strides,
    ...
    while 0 in filter_sizes:
        filter_sizes.remove(0)
...
class ConvVAE(object):
    ...
    def __init__(self, encoder_filters, encoder_kernel_sizes,
        ...
        if run_all:
            ...
            if plot_model:
                self.plot_model(save_name=parser.plot_name)

            self.train()

    def build_encoder(self, padding='same', activation='relu',
    ...
    def sampling(self, args, mean=0.0, stddev=1.0):
        if self.verbose:
            print('[INFO] Sampling Latent Layer')
    ...
    def build_decoder(self, padding='same', activation='relu',
    ...
    def plot_latent_representations(self, n_digits=15, digit_size=28,
        ...
        if self.verbose:
            print('[INFO] Plotting Latent Space Representations')
