...
def build_hidden_layers(hidden_dims, input_layer, Layer = None, 
	...
	if 'conv' in Layer.lower() and kernel_sizes is not None:
		...
		if 'conv1d' == Layer.lower(): Layer = Conv1D
		if 'conv2d' == Layer.lower(): Layer = Conv2D
		if 'conv3d' == Layer.lower(): Layer = Conv3D
		if 'conv1dT' == Layer.lower(): Layer = Conv1DTranspose
		if 'conv2dT' == Layer.lower(): Layer = Conv2DTranspose
		if 'conv3dT' == Layer.lower(): Layer = Conv3DTranspose
...
class VAEPredictor(object):
	...
	def build_predictor(self):
		if bool(sum(self.dnn_hidden_dims)):
			...
			dnn_hidden_layer = build_hidden_layers(Layer = 'Dense',
									...
									base_layer_name='dnn_hidden_layer', 
		...
		dnn_latent_mean = Dense(self.dnn_latent_dim, 
										name = 'dnn_latent_mean')
		self.dnn_latent_mean = dnn_latent_mean(dnn_hidden_layer)
		dnn_latent_log_var = Dense(self.dnn_latent_dim, 
									name = 'dnn_latent_log_var')
		
		self.dnn_latent_log_var = dnn_latent_log_var(dnn_hidden_layer)
		...
		dnn_prediction = Lambda(lambda tmp: tmp+1e-10, 
								name = 'dnn_prediction')
		self.dnn_prediction = dnn_prediction(self.dnn_latent_layer)
	...
	def build_model(self, batch_size = None, original_dim = None, 
		...
		if use_prev_input or self.use_prev_input:
			...
			out_stack = [self.vae_reconstruction, self.dnn_latent_layer, 
						 self.dnn_prediction, self.vae_latent_args]
	...
	def make_dnn_predictor(self):
		...
		# build label encoder
		enc_hidden_layer = self.model.get_layer('dnn_hidden_layer')
		enc_hidden_layer = enc_hidden_layer(input_layer)
		
		dnn_latent_mean = self.model.get_layer('dnn_latent_mean')
		dnn_latent_mean = dnn_latent_mean(enc_hidden_layer)

		dnn_latent_log_var = self.model.get_layer('dnn_latent_log_var')
