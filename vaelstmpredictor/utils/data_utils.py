...
class dummyData(object):
	...
	def __init__(self, batch_size):
		...
		features = amps*np.sin(2*np.pi /periods * (times - phases))
		labels = np.c_[amps, phases, periods]  # this may need to be `np.r_` instead of `np.c_` (?)

		(x_train, x_test, y_train, y_test) = train_test_split(features, labels, test_size=0.2)
		x_train_ = np.repeat(x_train, 2)
		x_test_ = np.repeat(x_test, 2)
		x_train = x_train_.reshape(x_train.shape+(2,))
		x_test = x_test_.reshape(x_test.shape+(2,))
