...
bincrossentropy = lambda x, xhat: (
    x * np.log(np.maximum(1e-15, xhat)) + (1 - x) * np.log(np.maximum(1e-15, 1 - xhat)))
...
class AnnealLossWeight(Callback):
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if epoch >= self.n_epochs:
            ...
            next_val = self.start_value + \
                self.next_weight(1.0 * epoch / self.n_epochs) * \
                (self.final_value - self.start_value)
