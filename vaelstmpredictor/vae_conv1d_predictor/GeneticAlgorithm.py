    ...
    while not all(generation.isTrained.values == 2):
        ...
        while True:
            try:
                ...
                message = 'tg1+query_generation failed because:{}'.format(
                    error)
                warning_message(message)
                sleep(sleep_time)
        ...
        if sql_generation is None:
            debug_message('sql_generation is None')
        if sql_generation is None:
            continue
...
def generate_ssh_command(clargs, chromosome):
    ...
    command.append('{} run_conv1d_chromosome.py '.format(which_python))
    ...
    if clargs.do_log:
        command.append('--do_log')
    if clargs.do_ckpt:
        command.append('--do_ckpt')
    if clargs.verbose:
        command.append('--verbose')
    if clargs.send_back:
        command.append('--send_back')
    if clargs.save_model:
...
def git_clone(hostname, username="acc", gitdir='vaelstmpredictor',
    ...
    command = []
    command.append(
        'git clone https://github.com/{}/{}'.format(gituser, gitdir))
