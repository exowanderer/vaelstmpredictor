...
# <<<<<<< HEAD:vaelstmpredictor/vae_predictor/train.py
...
def train_vae_predictor(clargs, data_instance, network_type='Dense'):
    ...
    callbacks = get_callbacks(clargs, patience=clargs.patience,
                              min_epoch=max(clargs.kl_anneal,
                                            clargs.w_kl_anneal) + 1,
    ...
    if network_type.lower() == 'dense':
        ...
        encoder_filters = np.array(
            [clargs.encoder_filter_size] * n_encoder_layers)
        ...
        decoder_filters = np.array(
            [clargs.decoder_filter_size] * n_decoder_layers)
